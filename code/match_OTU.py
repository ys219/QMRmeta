...
if __name__ == "__main__":
    ...
    # dereped input
    parser.add_argument("-d","--dereped_input", help = "dereplicated, quality filtered input reads", metavar = "INPUT.fasta/INPUT.fa")
    # otu before input
    parser.add_argument("-b","--unfiltered_otu", help = "OTUs for unfiltered(but quality filtered and dereplcated) reads", metavar = "unfiltered_OTU.fasta")
    # output file path with default
    parser.add_argument("-o","--output", help = "output filename (with default)", default = "./[INFO]OTU_info.csv", metavar = "OUTPUTFILENAME")# output
    # otu after input
    parser.add_argument("-a","--filtered_otu", help = "OTUs for filtered ASVs", metavar = "filtered_OTU.fasta")# 
    ...
    # OTU_before mapping
    subprocess.Popen("vsearch --usearch_global %s -db %s -id %s -blast6out %s"%(args.dereped_input,args.unfiltered_otu,args.id_otu,"[tmp]unfiltered_OTU_map.txt"),shell=True).wait()
    #
    bef_map = "[tmp]unfiltered_OTU_map.txt"
    #
    # 
    # OTU_after mapping
    subprocess.Popen("vsearch --usearch_global %s -db %s -id %s -blast6out %s"%(args.dereped_input,args.filtered_otu,args.id_otu,"[tmp]filtered_OTU_map.txt"),shell=True).wait()
    #
    aft_map = "[tmp]filtered_OTU_map.txt"
    #
    # 
    # OTUs_match_map
    subprocess.Popen("vsearch --search_exact %s -db %s -blast6out %s"%(args.unfiltered_otu,args.filtered_otu,"[tmp]OTUs_map.txt"),shell=True).wait()
    #
    ...
    #
    #save the output
    otu_df.T.to_csv(args.output,index=True)
